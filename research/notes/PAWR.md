...
14. **Colour sensor trigger on Brick 1.** You show the colour red to the brick's colour sensor. The DA000001-01 ASIC detects the colour internally and raises an interrupt on `[0xF00860]`. This is how Smart Bricks trigger actions — there is no physical button. (Green and blue presentations also fire ISR events, producing "repair" and "refuel" audio respectively — pure flavor, no state change. Only red additionally initiates a BrickNet send.)
15. **ISR `0x336E4` dispatches.** The ISR reads the ADC sample via AUX register 1680+channel×3, stashes it in the sensor event-queue struct at `0x8093E0`/`0x8093F0`, and emits a color-specific event code (one of 0x00, 0x02, 0x03, 0x04, 0x07, 0x08, 0x09, 0x0A) via callback dispatch at `0x33AD8`. For codes 0x00–0x05, the event feeds Queue A and is matched against the 38-row dispatch table at `0x37D5A4`; a script is selected and invoked via `0x6BFFE`. The selected script decodes a playback record from the active tag's combat banks — the local laser sound and LED flash play.
16. **BrickNet send from C, not bytecode.** The red-detection path additionally triggers a C-level BrickNet transmission. The `bricknet_send` opcode 21 is compiled dead code — no byte in the translation table maps to it in v0.72.1, so scripts cannot initiate BrickNet sends. The BrickNet coordinator code at `0x6D0E4` serializes 16 bytes of state into a 25-byte packet and transmits it. The exact call site from the color-detection path to `0x6D0E4` is not yet traced (`0x6D0E4` has zero static callers; it's invoked via function pointer).
...
21. **Hit counter increments (C code, two-threshold state machine).** The receive path hands the decoded message to `0x1232C`, which implements a two-threshold state machine on counter `[0x80CF28+0x12]` and flag bit 2 of `[0x80CF28+0x13]`. This is NOT script-driven — it's all C firmware.
    - Hits 1–3 (healthy): emit event `0x0D` / `0x0E` per hit onto Queue B.
    - Hit 4: set flag bit 2 ("damaged"), reset counter, emit event **`0x0F` (alarm)**.
    - Hit 5 (damaged): emit `0x0D` / `0x0E` per hit.
    - Hit 6 (2nd hit in damaged): clear flag bit 2, emit event **`0x0C` (destroyed / explosion)**.
22. **Handler plays sound synchronously.** The secondary dispatcher at `0x187F0` runs the handler for each emitted event — `0x18A12`/`0x18A3A` for individual hits, `0x18AB6` for alarm (event 0x0F), `0x1898E` for explosion (event 0x0C). Each handler updates state bits in `0x80CEBE` / `0x80DA14` / `0x8089FC` and plays the corresponding audio *synchronously* via direct play-engine calls (`0x4B9C`, `0x47344`/`0x47124`, `0x165E0`). The audio bank used is the one pointed to by the TIE Fighter's combat resource-reference record — that's why two different ships produce different hit/alarm/explosion sounds.
23. **Script #42 observes the damaged flag on its next tick.** Script #42 is already running on the receiving brick (same reason as Brick 1 — it's the type 0x0E script for PAwR-combat tags). On its next timer invocation, it reads flag bit 2 of `[0x80CF28+0x13]` and produces a branch-specific playback record: ambient while healthy, warning/damage loop while damaged, post-destruction cleanup after explosion.

**Observed gameplay timing:** alarm plays at the 4th hit, explosion at the ~6th hit (2 additional hits in damaged state). Matches the firmware logic exactly.

**Critical correction vs. earlier docs:** There is no "TIE Fighter response script that fires on hit." Hit / alarm / explosion reactions are all C-dispatched synchronously. The only script involved in combat is the already-running orchestrator (#42), which provides the continuous ambience and state-driven playback around the instantaneous C-dispatched event sounds. Earlier docs also mislabeled event `0x0F` as "explosion" and `0x0C` as "hit cleared"; the correct semantics are `0x0F` = alarm (damaged), `0x0C` = explosion (destroyed).
